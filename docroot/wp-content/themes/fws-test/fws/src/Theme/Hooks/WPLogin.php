...
class WPLogin extends SingletonHook
{
	...
	public function editLoginFooter(): void
	{
		...
		$html .= sprintf(
			'<div class="fws-footer-author"><span>' . __( 'Powered by ', 'fws_test' ) . '<a href="%s" target="_blank" rel="noopener">Forwardslash</a></span></div>',
			esc_url( 'http://fws.us/' )
		);
	...
	public function addLoginTitle($message): string
	{
		...
		if ( empty($message) ){
			$newHtml .=  '<span class="fws-login-title">' . __('Login to your account', 'fws_test') . '</span>';
		} else {
			$newHtml .= $message;
		}
	...
	public function editLoginFooterLink($html): string
	{
		...
		$htmlFooterText = sprintf(
			...
			__( 'Don\'t have an account?',  'fws_test' )
		);
