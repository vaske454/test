...
class SiteAssets extends SingletonHook
{
	...
	public function setupAdminStylesAndScripts(): void
	{
		...
		wp_enqueue_style( 'fws_test-admin-style', get_template_directory_uri() . '/dist/admin.css', [], $version );
		wp_enqueue_script( 'fws_test-admin-script', get_template_directory_uri() . '/dist/admin.min.js', ['jquery'], $version, false );

		wp_localize_script('fws_test-admin-script', $this->localizedObjectName, $this->localizedObjectValues);
	}
	...
	private function enqueueThemeStylesAndScripts(): void
	{
		...
		// Set Theme Site CSS
		wp_enqueue_style( 'fws_test-style', get_stylesheet_uri(), [], $version );

		// Set Theme Site JS
		wp_enqueue_script( 'fws_test-site-script', get_template_directory_uri() . '/dist/site.min.js', ['jquery'], $version, false );

		// Set Theme VueJS
		wp_enqueue_script( 'fws_test-vuevendors-js', get_template_directory_uri() . '/dist/vue-build/js/chunk-vendors.js', [], $version, false );
		wp_enqueue_script( 'fws_test-vueapp-js', get_template_directory_uri() . '/dist/vue-build/js/app.js', [], $version, false );

		// Localize JS Object
		wp_localize_script('fws_test-site-script', $this->localizedObjectName, $this->localizedObjectValues);
	}
